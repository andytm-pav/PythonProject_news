...
class EnhancedStockAIModel:
    ...
    def _create_new_models(self):
        """Создание новых моделей с улучшенными параметрами"""
        try:
            # Улучшенные параметры для RandomForest :cite[1]
            self.model = RandomForestRegressor(
                n_estimators=200,  # Увеличил количество деревьев
                max_depth=15,  # Увеличил глубину
                min_samples_split=10,  # Более консервативное разделение
            ...
            print("✅ ИИ модели созданы с улучшенными параметрами")
            return True
    ...
    def load_or_create_model(self):
        """Улучшенная загрузка моделей с валидацией"""
        try:
            if os.path.exists(self.model_path):
                print(f"🔄 Загрузка моделей из {self.model_path}")

                # Загружаем все компоненты модели
                model_data = joblib.load(self.model_path)

                self.model = model_data.get('main_model')
                self.probabilistic_model = model_data.get('probabilistic_model')
                ...
                if (self.model is not None and
                    ...
                    print("✅ Модели успешно загружены и валидированы")
                    return True
                else:
                    print("❌ Загруженные модели невалидны, создаем новые")
                    self._create_new_models()
                    return False

            else:
                print("📝 Файл моделей не найден, будут созданы новые")
                return False

        except Exception as e:
            print(f"⚠️ Ошибка загрузки моделей: {e}")
            print("🔄 Создаем новые модели...")
            self._create_new_models()
            return False
    ...
    def train_models(self, prices_df, news_sentiment_by_date):
        """Улучшенное обучение моделей с валидацией и мониторингом"""
        try:
            print("🔄 Запуск улучшенного обучения моделей...")
            ...
            print(
                f"📊 Данные для обучения: X.shape={X.shape if len(X) > 0 else 'empty'}, y.shape={y.shape if len(y) > 0 else 'empty'}")
            ...
            if len(X) < 50:  # Увеличиваем минимальный порог
                print(f"⚠️ Мало данных для обучения: {len(X)} примеров (требуется минимум 50)")
                return False

            # Проверяем размерности
            if X.shape[1] != 10:
                print(f"❌ Неправильная размерность признаков: {X.shape[1]} (ожидается 10)")
                return False

            print(f"🎯 Начало обучения на {len(X)} примерах...")

            # Разделяем на обучающую и тестовую выборки :cite[10]
            ...
            # Обучаем вероятностную модель
            print("   Обучение BayesianRidge...")
            self.probabilistic_model.fit(X_train, y_train)
            ...
            print(f"📈 Качество моделей:")
            print(f"   RandomForest R² (train): {train_score:.4f}")
            print(f"   RandomForest R² (test):  {test_score:.4f}")
            print(f"   MSE: {mse:.6f}")
            print(f"   MAE: {mae:.6f}")
            ...
            # Сохраняем полный набор моделей :cite[10]
            models_to_save = {
            ...
            print(f"📊 История обучения: {len(self.training_history)} записей")
...
class EnhancedStockAnalysisSystem:
    ...
    def _print_enhanced_results(self, recommendations, ai_predictions, trades, portfolio_value):
        ...
        if isinstance(model_info, dict):
            ...
            print(f"   Качество (R²): {model_info['test_score']:.4f}")
