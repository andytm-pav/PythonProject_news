...
class CorrectedStockDataCollector:
    ...
    def get_current_prices(self, tickers):
        ...
        for ticker in tickers:
            try:
                # ✅ ПРАВИЛЬНЫЙ endpoint для текущих данных
                ...
                if response.status_code == 200:
                    ...
                    if marketdata.get('data') and marketdata['data']:
                        last_price = marketdata['data'][0][1]  # LAST price
                        if last_price is not None:
                            current_prices[ticker] = float(last_price)
                            print(f"  ✅ {ticker}: {last_price} ₽ (текущая цена)")
                        else:
                            # Резервный вариант - цена закрытия предыдущего дня
                ...
                if response.status_code == 200:
                    ...
                    if candles:
                        ...
                        for candle in candles:
                            # ✅ ПРАВИЛЬНЫЕ ИНДЕКСЫ для candles
                            # candles структура: [open, close, high, low, value, volume, begin, end]
                            close_price = candle[1]  # ✅ close price
                            date_str = candle[6]  # ✅ begin date
...
# ==================== ИСПРАВЛЕННЫЙ КЛАСС ИИ МОДЕЛИ ====================
class CorrectedEnhancedStockAIModel:
    def __init__(self, model_path='corrected_stock_ai_model.joblib'):
    ...
    def _create_new_models(self):
        ...
        try:
            ...
            self.model_metadata = {
                ...
                'training_sessions': 0,
                'last_training': None,
                'model_type': 'CorrectedEnhancedStockAIModel_v1'
            }
    ...
    def prepare_training_data(self, prices_df, news_sentiment_by_date, forecast_days=10):
        """Подготовка данных для обучения с реальными целями"""
            ...
            try:
                if features and len(features) > 0:
                    ...
                    if len(price_series) > forecast_days:
                        # ✅ ПРАВИЛЬНЫЙ РАСЧЕТ ЦЕЛЕВОЙ ПЕРЕМЕННОЙ
                        # Используем исторические данные для обучения
                        current_idx = -1
...
# ==================== ИСПРАВЛЕННЫЙ ОСНОВНОЙ КЛАСС СИСТЕМЫ ====================
class CorrectedEnhancedStockAnalysisSystem:
    def __init__(self):
        self.news_collector = NewsCollector()
        self.stock_collector = CorrectedStockDataCollector()  # ✅ Исправленный коллектор
        self.news_analyzer = NewsAnalyzer()
        self.enhanced_ai_model = CorrectedEnhancedStockAIModel()  # ✅ Исправленная модель
    ...
    def run_enhanced_analysis_cycle(self):
        """ИСПРАВЛЕННЫЙ цикл анализа с правильными данными"""
        print("\n" + "=" * 60)
        ...
        try:
            ...
            print("🎯 Шаг 4: Обучение AI моделей на ИСТОРИЧЕСКИХ данных...")
            ...
            if self.enhanced_ai_model.is_trained:
                # ✅ Используем исторические данные для предсказаний
                ...
                if optimal_weights:
                    # ✅ Используем ТЕКУЩИЕ цены для ребалансировки портфеля
            ...
            # ✅ Используем ТЕКУЩИЕ цены для расчета стоимости портфеля
    ...
    def run_continuous_analysis(self, interval_minutes=3):
        ...
        print("🚀 Запуск ИСПРАВЛЕННОЙ системы анализа российских акций с ИИ...")
...
if __name__ == "__main__":
    system = CorrectedEnhancedStockAnalysisSystem()
