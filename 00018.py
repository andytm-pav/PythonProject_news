...
class EnhancedStockAIModel:
    def __init__(self, model_path='enhanced_stock_ai_model.joblib'):
        ...
        self.imputer = SimpleImputer(strategy='median')  # Импьютер для обработки NaN
    ...
    def _create_new_models(self):
        ...
        try:
            # Основная модель для базовых прогнозов
            ...
            # Улучшенная модель для прогнозирования доходности
            ...
            # Инициализация импьютера и скейлера
            ...
            self.model_metadata = {
                ...
                'training_sessions': 0,
                'last_training': None,
                'model_type': 'EnhancedStockAIModel_v7'
            }
    ...
    def _clean_features(self, features_dict):
        ...
        for ticker, features in features_dict.items():
            try:
                # Преобразуем в numpy array и проверяем на валидность
                ...
                # Проверяем, что нет NaN после преобразования
            ...
            try:
                if features and len(features) > 0:
                    ...
                    if len(price_series) > forecast_days:
                        ...
                        future_price = price_series.iloc[-forecast_days - 1] if len(price_series) > forecast_days else \
                        price_series.iloc[0]
    ...
    def optimize_portfolio(self, predictions, prices_df, risk_aversion=1.0):
        ...
        try:
            ...
            def objective(weights):
                ...
                return -sharpe_ratio  # Минимизируем отрицательный Sharpe Ratio
    ...
    def fetch_all_news(self, max_items_per_feed=15):
        ...
        for source, url in self.feeds.items():
            try:
                ...
                time.sleep(0.3)  # Уменьшил задержку
...
class StockDataCollector:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if candles:
                        ...
                        if len(prices) >= 10:  # Минимум 10 точек данных
    ...
    def create_test_data(self, tickers, days):
        ...
        for ticker in tickers:
            ...
            # Реалистичная симуляция с трендами и волатильностью
...
class VirtualPortfolio:
    ...
    def rebalance_portfolio(self, target_weights, current_prices, max_position_size=0.3):
        ...
        executed_trades = []
        transaction_cost = 0.001  # 0.1% комиссия

        # Продажа активов, превышающих целевой вес
        for ticker in list(self.positions.keys()):
            if ticker in current_prices and current_prices[ticker] > 0:
                ...
                # Продаем если текущий вес больше целевого на 5%
        ...
        # Покупка активов согласно целевым весам
        for ticker, target_weight in target_weights.items():
            if target_weight > 0 and ticker in current_prices and current_prices[ticker] > 0:
                ...
                # Покупаем если текущий вес меньше целевого на 5%
...
class WebInterface:
    ...
    def get_html_template(self):
        ...
        <html>
        <head>
            <title>Улучшенный анализ российских акций с ИИ</title>
            <meta charset="UTF-8">
            <style>
        ...
        <body>
            <div class="container">
                <div class="header">
                    <h1>🤖 Улучшенный анализ российских акций с ИИ</h1>
...
class EnhancedStockAnalysisSystem:
    ...
    def run_enhanced_analysis_cycle(self):
        ...
        try:
            ...
            # 4. AI анализ и оптимизация портфеля
            ...
            # Информация о модели для отображения
            model_info = {
    ...
    def _enhance_recommendations_with_optimization(self, news_recommendations, optimal_weights):
        ...
        if optimal_weights:
            ...
            # Преобразуем веса в рейтинги -5 до +5
            if sector_weights:
                max_weight = max(sector_weights.values())
                for sector in enhanced_recommendations:
                    weight = sector_weights.get(sector, 0)
                    if max_weight > 0:
                        optimization_score = (weight / max_weight) * 10 - 5  # -5 до +5
                        # Комбинируем новостной анализ и оптимизацию
