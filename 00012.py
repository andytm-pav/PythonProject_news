...
# ==================== РАСШИРЕННЫЙ КЛАСС ИИ МОДЕЛИ (ИСПРАВЛЕННЫЙ) ====================
class EnhancedStockAIModel:
    def __init__(self, model_path='enhanced_stock_ai_model.joblib'):
        ...
        self._create_new_models()  # Сразу создаем модели при инициализации

    def _create_new_models(self):
        """Создание новых моделей - гарантирует, что модели всегда существуют"""
        try:
    ...
    def load_or_create_model(self):
        """Загрузка или создание ансамбля моделей - УПРОЩЕННАЯ ВЕРСИЯ"""
        try:
            if os.path.exists(self.model_path):
                ...
                print("📝 Файл модели не найден, используются новые модели")
                return False
        except Exception as e:
            print(f"⚠️ Ошибка загрузки моделей: {e}, используются новые модели")
            return False

    def create_advanced_features(self, prices_df, news_sentiment):
        """Создание расширенных признаков для моделей - УЛУЧШЕННАЯ ВЕРСИЯ"""
        features = {}
        ...
        for ticker in prices_df.columns:
            try:
                ...
                if len(price_series) < 20:  # Увеличил минимальное количество точек
                    continue
                ...
                # Волатильность
                volatility = returns.rolling(window=10, min_periods=5).std().iloc[-1] if len(returns) >= 5 else 0.02
                ...
                if not loss.iloc[-1] == 0:
                    rs = gain.iloc[-1] / loss.iloc[-1]
                    rsi = 100 - (100 / (1 + rs))
                rsi = max(0, min(100, rsi))  # Ограничиваем RSI

                # Моментум
                momentum = 0.0
                if len(price_series) >= 5:
                    momentum = (price_series.iloc[-1] / price_series.iloc[-5] - 1)
    ...
    def prepare_training_data(self, prices_df, news_sentiment_by_date, forecast_days=5):
        """Подготовка данных для обучения - УЛУЧШЕННАЯ ВЕРСИЯ"""
        X, y = [], []
        ...
        for ticker in prices_df.columns:
            try:
                ...
                if len(price_data) < 30:  # Увеличил минимальную длину
                    continue

                # Используем последний доступный сентимент для всех дат
                current_sentiment = {}
                if news_sentiment_by_date:
                    # Берем последний доступный сентимент
                ...
                for i in range(25, len(price_data) - forecast_days):
                    try:
                        ...
                        if ticker in features_dict and features_dict[ticker]:
                            ...
                            # Проверяем валидность признаков
                            if (len(feature_vector) == 5 and
                                    all(not pd.isna(x) for x in feature_vector) and
                                    all(np.isfinite(x) for x in feature_vector)):
    ...
    def train_models(self, prices_df, news_sentiment_by_date):
        ...
        try:
            ...
            if len(X) < 20:  # Минимум 20 примеров
                print(f"⚠️ Мало данных для обучения: {len(X)} примеров (требуется минимум 20)")
                # Временно создаем тестовые данные для демонстрации
            ...
            if X.shape[1] != 5:
                print(f"❌ Неправильная размерность признаков: {X.shape[1]} (ожидается 5)")
                return False
            ...
            print(f"💾 Модели успешно обучены и сохранены в {self.model_path}")
            return True
    ...
    def predict_with_confidence(self, current_features):
        """Прогноз с оценкой достоверности - ИСПРАВЛЕННАЯ ВЕРСИЯ"""
        if not self.is_trained or self.model is None:
            print("⚠️ Модель не обучена, возвращаем нулевой прогноз")
            return 0.0, 1.0  # Возвращаем высокую неопределенность
        ...
        try:
            ...
            return float(combined_pred), float(probabilistic_std[0])
...
# ==================== ОСНОВНОЙ КЛАСС СИСТЕМЫ (ИСПРАВЛЕННЫЙ) ====================
class EnhancedStockAnalysisSystem:
    def __init__(self):
        self.news_collector = NewsCollector()
        self.stock_collector = StockDataCollector()
        self.news_analyzer = NewsAnalyzer()
        self.enhanced_ai_model = EnhancedStockAIModel()  # Теперь модели создаются сразу
    ...
    def run_enhanced_analysis_cycle(self):
        """Расширенный цикл анализа с AI прогнозами и ребалансировкой - ИСПРАВЛЕННАЯ ВЕРСИЯ"""
        print("\n" + "=" * 60)
        ...
        try:
            ...
            # 3. Обучение моделей (ВЫЗЫВАЕТСЯ ПРАВИЛЬНО)
            ...
            # Обучаем только если модель не была загружена или требует дообучения
            if not model_loaded or not self.enhanced_ai_model.is_trained:
                print("🔄 Обучение новых моделей...")
                is_trained = self.enhanced_ai_model.train_models(prices_df, news_sentiment_by_date)
                if not is_trained:
                    print("⚠️ Модель не обучена, будут использоваться базовые предсказания")
            else:
                print("✅ Используется предварительно обученная модель")
    ...
    def _print_enhanced_results(self, recommendations, ai_predictions, trades, portfolio_value):
        ...
        for ticker, data in sorted_predictions:
            ...
            # ИСПРАВЛЕНИЕ: Форматируем отдельные числа, а не массивы
        ...
        # ИСПРАВЛЕНИЕ: Форматируем отдельное число
        ...
        while self.web_interface.is_running:
            try:
                ...
                if success:
                    ...
                    for i in range(interval_minutes * 60):  # Исправлено: 60 секунд в минуте
...
# Остальные классы (NewsCollector, StockDataCollector, NewsAnalyzer, VirtualPortfolio,
# PortfolioRebalancer, WebInterface) остаются без изменений, как в вашем исходном коде
