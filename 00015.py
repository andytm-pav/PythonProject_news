...
class EnhancedStockAIModel:
    def __init__(self, model_path='enhanced_stock_ai_model.joblib'):
        ...
        # Гарантированная инициализация моделей при создании
        self._initialize_models()

    def _initialize_models(self):
        """Гарантированная инициализация моделей"""
        print("🔄 Инициализация ИИ моделей...")

        # Всегда создаем новые модели, если не удалось загрузить существующие
        if not self._try_load_existing_model():
            self._create_new_models()
            print("✅ Созданы новые модели")
        else:
            print("✅ Модели загружены из файла")

    def _try_load_existing_model(self):
        """Попытка загрузки существующей модели с улучшенной обработкой ошибок"""
        try:
            if not os.path.exists(self.model_path):
                print("📝 Файл модели не найден, создаем новые модели")
                return False
    ...
    def _create_new_models(self):
        ...
        try:
            ...
            self.model = RandomForestRegressor(
                n_estimators=100,  # Уменьшено для быстродействия
                max_depth=10,
                min_samples_split=5,
                min_samples_leaf=2,
                random_state=42,
            ...
            self.model_metadata = {
                ...
                'training_sessions': 0,
                'last_training': None,
                'model_type': 'EnhancedStockAIModel_v4'
            }
    ...
    def train_models(self, prices_df, news_sentiment_by_date, incremental=True):
        """Улучшенное обучение моделей с проверкой инициализации"""
        try:
            print("🔄 Запуск обучения моделей...")

            # КРИТИЧЕСКИ ВАЖНО: Проверяем инициализацию моделей
            if self.model is None or self.probabilistic_model is None:
                ...
                # Двойная проверка
                if self.model is None or self.probabilistic_model is None:
                    print("❌ Не удалось инициализировать модели после создания")
                    return False
            ...
            # Обработка данных
            try:
                if hasattr(self.scaler, 'n_features_in_') and incremental:
            ...
            # ОБУЧЕНИЕ С ГАРАНТИЕЙ: проверяем модели перед вызовом fit
            if not hasattr(self.model, 'fit'):
                print("❌ Основная модель не имеет метода fit")
                return False

            if not hasattr(self.probabilistic_model, 'fit'):
                print("❌ Вероятностная модель не имеет метода fit")
                return False
            ...
            # Обновляем статус
            self.is_trained = True
            self._update_training_metadata(len(X), training_type="full")
...
class EnhancedStockAnalysisSystem:
    ...
    def run_enhanced_analysis_cycle(self):
        ...
        try:
            ...
            # 3. Обучение AI моделей - ТЕПЕРЬ С ГАРАНТИЕЙ
